...
In this section we show the processing of example samples from a spike-in experiment designed for interrogating data processing methods from the **[gcspikelite](http://bioconductor.org/packages/release/data/experiment/html/gcspikelite.html)** data package [@robinson2018] that were analyzed using GC–MS. 
...
**eRah** also supports parallel processing at this step to enable faster processing of large sample sets.
This requires a list containing two options:

* `nCores` - number of processors to use
* `clusterType` - the cluster type as supported by `makeCluster()` in the **parallel** package

These options can be added to a parallel parameters list ready to be passed to `deconvolveComp()`.
The following allows for processing across two processors using a PSOCK cluster:
